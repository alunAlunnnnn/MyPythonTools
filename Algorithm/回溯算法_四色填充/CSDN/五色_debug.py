...
def generateColorValue(InputFeature, level, UniqueField, ConnectField, maxColor):
    U = []
    C = []
    S = []
    N = 0  # 图层中多边形的个数
...
def main(InputFeature, colorField, level, UniqueField, ConnectField, maxColor):
    # add a corlor filed
    addColorField(InputFeature, colorField)

    # calculate color value
    generateColorValue(InputFeature, level, UniqueField, ConnectField, maxColor)


# InputFeature = r"F:\杂七杂八\学习资料\ArcGIS学习：用Python实现地图四色填充（数据与工具）\实例数据\china_5_fen_new3.shp"
# colorField = ""
# UniqueField = "unicid"
...
maxColor = arcpy.GetParameterAsText(5)

main(InputFeature, colorField, level, UniqueField, ConnectField, maxColor)
