...
def getAttrFromFC(inFC):
    ...
    coordFieldList = ['x_temp_c', 'y_temp_c', 'z_temp_c']
    ...
    dataSorted = arcpy.Sort_management(inFC, 'in_memory/data_2d_', [['x_temp_', 'Ascending'], ['y_temp_', 'Ascending'], ['z_temp_', 'Ascending']])
...
field = ['x_temp_', 'y_temp_', 'z_temp']
fieldCal = ['!shape.centroid.X!', '!shape.centroid.Y!', '!shape.centroid.Z!']
for i, each in enumerate(field):
    arcpy.AddField_management(data2d, each, 'DOUBLE')
    arcpy.AddField_management(data3d, each, 'DOUBLE')
    arcpy.CalculateField_management(data2d, each, fieldCal[i], 'PYTHON3')
    arcpy.CalculateField_management(data3d, each, fieldCal[i], 'PYTHON3')
