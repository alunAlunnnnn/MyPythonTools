...
def showPoints(tabGDB, tabset, resGDB, fieldX, fieldY, fieldH, spatialReferencePRJ, tabNameMapping):
    ...
    for each in tabset:
        ...
        if count:
            ...
            if fieldH:
                pntLayer = arcpy.MakeXYEventLayer_management(pntTabLayer, fieldX, fieldY, "pntLayer", spatialReferencePRJ, fieldH)
            else:
                pntLayer = arcpy.MakeXYEventLayer_management(pntTabLayer, fieldX, fieldY, "pntLayer", spatialReferencePRJ)
...
def tab2ply(tabGDB, tabset, resGDB, conAttr, fieldX, fieldY, fieldH, spatialReferencePRJ, tabNameMapping):
    resTotal = {"notEmpty": [], "empty": []}
    arcpy.env.workspace = tabGDB
    for each in tabset:
        plyTab = each + '1'
        ...
        if count:
            ...
            plyResWithAttr = os.path.join(resGDB, tabNameMapping[plyTab])
...
fieldH = arcpy.GetParameterAsText(6)
tabNameTxt = arcpy.GetParameterAsText(7)
fieldSerialNumber = arcpy.GetParameterAsText(8)
conAttr = arcpy.GetParameterAsText(9)
...
tab2ply(gdb, tabSet, resGDB, conAttr, fieldX, fieldY, fieldH, spatialReferencePRJ, tabNameMapping)
