...
def tab2plg(tabGDB, tabset, resGDB, fieldSerialNumber, spatialReferencePRJ, tabNameMapping):
    ...
    for each in tabset:
        ...
        if count:
            ...
            plgTemp_ = os.path.join(resGDB, "plg_%s_temp" % plgTab)
            ...
            if int(arcpy.GetCount_management(pntTemp)[0]):
                ...
                # join the id to polygon from ply
                plgTemp = arcpy.SpatialJoin_analysis(plgRes, resPly, plgTemp_)

                # add field to plg table to connect with plg feature class
                arcpy.AddMessage(plgTab)
                plgView = arcpy.MakeTableView_management(plgTab, "tabview")

                _AddField(plgView, "BBB", "TEXT")

                arcpy.CalculateField_management(plgView, "BBB", "!Points![:17]", "PYTHON_9.3")
                arcpy.AddMessage([each.name for each in arcpy.ListFields(plgView)])

                plyr = arcpy.MakeFeatureLayer_management(plgTemp_, "plglyr")

                # set field name
                arcpy.env.qualifiedFieldNames = False

                arcpy.AddJoin_management(plyr, "BBB", plgView, "BBB")

                arcpy.CopyFeatures_management(plyr, plgResSpatialJoin)

                # try:
                #     arcpy.DeleteField_management(plgResSpatialJoin, "BBB")
                #     arcpy.DeleteField_management(plgResSpatialJoin, "BBB_1")
                #     arcpy.DeleteField_management(plgResSpatialJoin, "AAA")
                #     arcpy.DeleteField_management(plgResSpatialJoin, "B")
                # except:
                #     pass
                #
                # try:
            ...
            arcpy.AddMessage("temp data, deleting.....")
            tempDataList = [pntTemp, plgRes, pntRes, plyRes, plgTemp_]
...
templatePntTable = arcpy.GetParameterAsText(3)
fieldSerialNumber = arcpy.GetParameterAsText(4)
spatialReferencePRJ = arcpy.GetParameterAsText(5)
tabNameTxt = arcpy.GetParameterAsText(6)
