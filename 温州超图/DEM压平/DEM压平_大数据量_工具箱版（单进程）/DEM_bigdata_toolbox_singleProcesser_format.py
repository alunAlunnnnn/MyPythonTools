...
def getRunTime(func):
    ...
    def _wrapper(*args, **kwargs):
        ...
        print("Start run function {}, at {}".format(func.__name__, start))
        ...
        print("*" * 16)
        print("Function {} run infomation".format(func.__name__, ))
...
def _addConvertField(inFC):
    ...
    if len([each.name for each in arcpy.ListFields(inFC) if each.name == "HSCONF_"]) > 0:
        ...
        print("No Such Field Named HSCONF_ In Feature Class {}".format(inFC))
        raise NoFieldError
...
def main(inRas, inPlg, outputPath, outputName):
    ...
    for eachValue in oIDValueList:
        ...
        # select each feature
        selLyr = arcpy.SelectLayerByAttribute_management(plgLyr, "NEW_SELECTION", "{} = {}".format(oIDName, eachValue))

        # convert eachplg to raster
        print(myRas.format)
        tmpRas = os.path.join(tmpDir, "ras_{}{}".format(eachValue, myRas.format))
        print(tmpRas)
        ...
        demTimeSave = os.path.join(tmpDir, "demTime_{}{}".format(eachValue, myRas.format))
        ...
        tmpMinRas = os.path.join(tmpDir, "min_{}{}".format(eachValue, myRas.format))
        minusRas.save(tmpMinRas)

        singleRasList.append(tmpMinRas)

    # mosaic all raster to one
    arcpy.MosaicToNewRaster_management(singleRasList, tmpDir, "mergedRaster.img", pixel_type="32_BIT_FLOAT",
                                       number_of_bands=1)
    mosaicRaster = os.path.join(tmpDir, "mergedRaster.img")

    # # set fill None extent
    arcpy.env.extent = myRas.extentObj

    # fill null data with zero to single raster
    minFillRas = arcpy.sa.Con(arcpy.sa.IsNull(mosaicRaster), 0, mosaicRaster)

    # dem plus total process raster
    resDem = os.path.join(outputPath, outputName)
    resDemObj = arcpy.sa.Plus(inRas, minFillRas)
