...
def main(inRas, inPlg, outputPath, outputName):
    ...
    arcpy.SetProgressor("step", "processing...", 0, len(oIDValueList), 1)
    print(f"start run total {len(oIDValueList)}...")

    plgLyr = arcpy.MakeFeatureLayer_management(inPlg)
    for _, eachValue in enumerate(oIDValueList):
        # set processing
        arcpy.SetProgressorPosition()
        print(f"running {_}/{len(oIDValueList)}...")
